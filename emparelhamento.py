...
class Hungaro:
	...
	def lerGrafoDoArquivoMatrizAdjacente(self, caminho):
			...
			if linha!='':
				...
				if i==1:
					...
					for no in range(linha.count('\t')):
						...
						contagem+=1
					grafo.add_nodes(lista_no)
				j = 1
				for cell in lista:
					if cell == '1' and j>i:
						grafo.add_edge((''+str(j), ''+str(i)))
		...
		img.write_gif('grafoBipartido.gif')
